...
import { getProfiles, getPublications, getProfileInterests } from './queries'
...
export const APIURLV2 = "https://api-v2.lens.dev"
...
export async function fetchProfileInterests(id) {
  try {
    const urqlClient = await createClientV2()
    console.log("id", id )
    const returnedProfile = await urqlClient.profile.fetch({
      forProfileId: id,
    })
    ;
    const profileData = returnedProfile.data
    console.log("returned", returnedProfile)
    console.log(profileData)
    return profileData.profileInterests
  } catch (err) {
    console.log('error fetching profile...', err)
  }
}
